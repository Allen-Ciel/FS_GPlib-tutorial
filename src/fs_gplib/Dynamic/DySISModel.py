...
class DySISModel(DiffusionModel):
    r"""Dynamic SIS (DySIS) epidemic model on **time-varying networks** (dynamic network models).

    This model extends the classical static SIS dynamics (see the epidemic ``SISModel``) to a
    sequence of graph snapshots :math:`\{G^{(k)}=(V,E^{(k)})\}_{k=1}^{T}`.  At step :math:`k`,
    a susceptible node may be infected by infected neighbors in :math:`N^{(k)}(i)` with probability
    :math:`\beta` per contact (optionally scaled by snapshot edge weights in *edge_attr_list*),
    and an infected node may recover to susceptible with probability :math:`\lambda`.

    The number of simulation steps cannot exceed ``len(edge_index_list)``.  As with other dynamic
    models, pass an explicit node tensor *x* (shape ``(N, 1)``) and *edge_index_list* (and optional
    *edge_attr_list*) rather than a single PyG ``Data`` object.
