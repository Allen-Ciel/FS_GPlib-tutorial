...
class DySEISModel(DiffusionModel):
    r"""Dynamic SEIS (DySEIS) epidemic model on **time-varying networks**.

    This model extends the classical static SEIS dynamics to a sequence of graph
    snapshots :math:`\{G^{(k)}=(V,E^{(k)})\}_{k=1}^{T}`. Nodes progress through
    susceptible :math:`S`, exposed :math:`E`, and infectious :math:`I`.
    At snapshot :math:`k`, infectious neighbors in :math:`N^{(k)}(i)` expose a
    susceptible node with probability :math:`\beta` per contact (optionally
    scaled by snapshot edge weights in *edge_attr_list*); exposed nodes become
    infectious with probability :math:`\alpha` (*latent_alpha*); infectious
    nodes recover back to susceptible with probability :math:`\gamma`
    (*removal_gamma*).

    Returned tensors encode states as **float** values: susceptible ``0``, 
    infected ``1``, exposed ``2``.

    The number of simulation steps cannot exceed ``len(edge_index_list)``. Pass
    an explicit node tensor *x* (shape ``(N, 1)``) and *edge_index_list* (and
    optional *edge_attr_list*) instead of a single PyG ``Data`` object.

    :param x: Node feature tensor of shape ``(N, 1)`` (node count :math:`N`
        from the leading dimension).
    :type x: torch.Tensor
    :param edge_index_list: One ``edge_index`` tensor per snapshot, length
    ...
    :param infection_beta: Exposure probability :math:`\beta \in [0, 1]` for
        each infectious neighbor (S→E).
    :type infection_beta: float
    :param latent_alpha: Latent progression probability :math:`\alpha \in [0, 1]`
        (E→I).
    :type latent_alpha: float
    :param removal_gamma: Recovery probability :math:`\gamma \in [0, 1]`
        (I→S).
    ...
    :param edge_attr_list: *(optional)* One edge-weight tensor per snapshot,
        aligned with *edge_index_list*. If ``None``, weights are ``1``.
