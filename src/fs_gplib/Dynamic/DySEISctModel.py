...
class DySEISctModel(DiffusionModel):
    r"""Dynamic SEIS with continuous-time hazards (DySEIS_ct) on time-varying networks.

    This model runs SEIS diffusion on a snapshot sequence
    :math:`\{G^{(k)}=(V,E^{(k)})\}_{k=1}^{T}`. The exposure mechanism for
    :math:`S\to E` follows the discrete DySEIS model: infectious neighbors in
    ...
    Unlike DySEIS, transitions :math:`E\to I` and :math:`I\to S` are governed
    by **elapsed-time dependent** probabilities. If a node entered state
    :math:`E` at iteration :math:`t_i^E` and state :math:`I` at iteration
    :math:`t_i^I`, then at snapshot index :math:`k`:

    - :math:`P(E\to I)=1-\exp(-\alpha\,(k-t_i^E))`
    - :math:`P(I\to S)=1-\exp(-\gamma\,(k-t_i^I))`

    Returned tensors encode states as **float** values: susceptible ``0``, 
    infected ``1``, exposed ``2``.
    ...
    :param seeds: Initially infectious nodes: list of node IDs or a float in
        ``[0,1)``.
    :type seeds: list[int] | float
    :param infection_beta: Exposure probability :math:`\beta` (S→E), in
        ``[0,1]``.
    :type infection_beta: float
    :param latent_alpha: Latent parameter :math:`\alpha` for E→I, in
        ``[0,1]``.
    :type latent_alpha: float
    :param removal_gamma: Removal parameter :math:`\gamma` for I→S, in
        ``[0,1]``.
    ...
    def _set_device(self, device):
        ...
        self.model = DySEISctModel_process(self.edge_index_list, self.edge_attr_list, self.infection_beta, self.removal_gamma, self.latent_alpha).to(self.device)
    ...
    def run_iterations(self, times):
        ...
        x_list, E_mask_list, E_iteration_list, I_iteration_list = self.model(self.node_status, iterations_times=times)

        out_all = torch.stack(x_list, dim=0), torch.stack(E_mask_list, dim=0), torch.stack(E_iteration_list, dim=0), torch.stack(I_iteration_list, dim=0)
        self.node_status['SI'], self.node_status['E_mask'], self.node_status['E_iteration'], self.node_status['I_iteration'] = out_all[0][-1].squeeze(0), out_all[1][-1].squeeze(0), out_all[2][-1].squeeze(0), out_all[3][-1].squeeze(0)
    ...
    def run_epoch(self):
        ...
        **re-initialised** before the epoch starts.
        ...
        :return: Node states trajectory over all snapshots, shape ``(T, 1, N)``
            with :math:`T =` ``len(edge_index_list)`` (values ``0``–``2``).
        :rtype: torch.Tensor
        """
    ...
    def run_epochs(self, epochs, batch_size=200):
        ...
        the epidemic is evolved through **all** snapshots. Node states are
        **re-initialised** before the run.
        ...
        with torch.no_grad():
            for i, epoch_group in enumerate(bar):
                ...
                out_list, E_mask_list, _, _ = self.model(self.node_status, epoch_group)
                out_all = torch.stack(out_list, dim=0), torch.stack(E_mask_list, dim=0), None, None
                final = self._return_final(out_all)
...
class DySEISctModel_process(Diffusion_process):
    def __init__(self,
                 edge_index_list,
                 edge_attr_list,
                 infection_beta,
    ...
    def forward(self, node_status, epochs=1, iterations_times = None):
        ...
        while iterations_times > self.times:
            ...
            if self.edge_attr_list is not None:
                self.edge_attr = self.edge_attr_list[self.times].to(self.device).unsqueeze(0).unsqueeze(2).to(self.device)
            else:
        ...
        return x_list, E_mask_list, E_iteration_list, I_iteration_list
