...
class SEIRModel(DiffusionModel):
    r"""SEIR (Susceptible-Exposed-Infected-Recovered) diffusion model on static graphs.

    Each node starts as susceptible or infected (seed).  At every step each
    infected neighbor independently exposes a susceptible node with probability
    :math:`\beta` (S→E), exposed nodes become infectious with probability
    :math:`\alpha` (E→I), and infected nodes recover with probability
    :math:`\gamma` (I→R) and become permanently immune.

    Returned node states are encoded as: 0 = susceptible, 1 = infected,
    2 = exposed, 3 = recovered.
    ...
    :param removal_gamma: Per-step recovery probability
        :math:`\gamma \in [0,1]` (I→R).
    :type removal_gamma: float
    :param latent_alpha: Per-step incubation/progression probability
        :math:`\alpha \in [0,1]` (E→I).
    ...
    def _set_device(self, device):
        ...
        if self.use_weight:
            self.model = SEIR_process(self.data.edge_index, self.infection_beta, self.removal_gamma, self.latent_alpha, self.data.edge_attr)
        else:
    ...
    def run_iterations(self, times):
        ...
        self.node_status['SI'], self.node_status['E_mask'], self.node_status['R_mask'] = out_all[0].squeeze(0), out_all[1].squeeze(0), out_all[2].squeeze(0)
    ...
    # SEIR based on Message Passing
class SEIR_process(Diffusion_process):
    def __init__(self,
                 edge_index,
                 infection_beta,
                 removal_gamma,
