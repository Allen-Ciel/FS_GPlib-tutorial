...
class SEISModel(DiffusionModel):
    r"""SEIS (Susceptible-Exposed-Infected-Susceptible) diffusion model on static graphs.
    ...
    :math:`\alpha` (E→I), and infected nodes recover back to susceptible with
    probability :math:`\gamma` (I→S).
    ...
    def _set_device(self, device):
        ...
        if self.use_weight:
            self.model = SEIS_process(self.data.edge_index, self.infection_beta, self.removal_gamma, self.latent_alpha, self.data.edge_attr)
        else:
            self.model = SEIS_process(self.data.edge_index, self.infection_beta, self.removal_gamma, self.latent_alpha, None)
    ...
    def run_iterations(self, times):
        ...
        self.node_status['SI'], self.node_status['E_mask'] = out_all[0].squeeze(0), out_all[1].squeeze(0)
...
class SEIS_process(Diffusion_process):
    def __init__(self,
                 edge_index,
                 infection_beta,
                 removal_gamma,
