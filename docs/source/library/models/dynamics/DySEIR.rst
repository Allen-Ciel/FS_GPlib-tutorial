...
The DySEIR (Dynamic SEIR) Model extends the classical SEIR model to **time-varying networks**.
...
Each node can be in one of four states: :math:`S` (susceptible), :math:`E` (exposed/incubating),
:math:`I` (infected), or :math:`R` (recovered).
...
:math:`\alpha`; infected nodes recover with rate :math:`\gamma`:
...
.. math::
   ...
   \frac{dR_i}{dt}\bigg|_{t=k} &= \gamma I_i .
   \end{aligned}


Compared with the static :doc:`SEIR <../epidemics/SEIR>` model on a single graph,
the DySEIR model captures the impact of **evolving topology** on exposure, infection,
...
+----------------+------------------------------+---------------+-----------+--------------------------------------------------+
| latent_alpha   | float in [0, 1]              |               | Yes       | Progression probability (E→I).                   |
+----------------+------------------------------+---------------+-----------+--------------------------------------------------+
| removal_gamma  | float in [0, 1]              |               | Yes       | Recovery probability (I→R).                      |
+----------------+------------------------------+---------------+-----------+--------------------------------------------------+
...
.. note::
   ...
   Unlike the static SEIR model which accepts a single ``data`` object containing
   ``edge_index`` and ``edge_attr``, the DySEIR model requires an explicit node
...
3) an :math:`I` state node is recovered to :math:`R` with probability :math:`\gamma` (I→R).

Node states are represented by three Boolean indicator vectors :math:`h, e, r \in \{0,1\}^N`,
where :math:`h_i=1` denotes ``infected`` or ``exposed``, :math:`h_i=0` denotes ``susceptible`` or ``recovered``,
:math:`e_i=1` denotes ``exposed``, and :math:`r_i=1` denotes ``recovered``.
Therefore :math:`(h_i,e_i,r_i)=(0,0,0)` is ``susceptible``, :math:`(1,1,0)` is ``exposed``,
:math:`(1,0,0)` is ``infected``, and :math:`(0,0,1)` is ``recovered``.
...
.. math::
   ...
   h_i^{(k)} &=
   \begin{cases}
      1, & \text{if } (h_i^{(k-1)}=0 \land r_i^{(k-1)}=0) \land (U_i^{\mathrm{exp}} < m_i^{(k)}), \\[4pt]
   ...
   r_i^{(k)} &=
   \begin{cases}
      1, & \text{if } (h_i^{(k-1)}=1 \land e_i^{(k-1)}=0) \land (U_i^{\mathrm{rec}} < \gamma), \\[4pt]
