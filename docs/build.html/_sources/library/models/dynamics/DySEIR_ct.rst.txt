...
The DySEIR_ct (Dynamic SEIR with continuous-time transition hazards) model extends
the classical SEIR process to **time-varying networks**.
...
Each node can be in one of four states: :math:`S` (susceptible), :math:`E` (exposed),
:math:`I` (infected), or :math:`R` (recovered).

Compared with the discrete DySEIR model, DySEIR_ct keeps the same :math:`S\to E`
infection mechanism but uses **elapsed-time dependent transition probabilities** for
:math:`E\to I` and :math:`I\to R`.
...
.. math::
   ...
   P(I\to R) = 1-\exp(-\gamma\Delta t_i^I).

This design captures non-memoryless progression and recovery effects while still
running on snapshot-based dynamic contact networks.
...
+----------------+------------------------------+---------------+-----------+--------------------------------------------------+
| removal_gamma  | float in [0, 1]              |               | Yes       | Hazard parameter for I→R recovery.               |
+----------------+------------------------------+---------------+-----------+--------------------------------------------------+
...
.. note::
   ...
   Unlike the static :doc:`SEIR_ct <../epidemics/SEIR_ct>` model which accepts a
   single graph object, DySEIR_ct requires a node tensor ``x`` and a **snapshot list**
...
Node states are represented by three Boolean vectors :math:`h,e,r \in \{0,1\}^N`
